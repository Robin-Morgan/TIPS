...
# run scripts in the director:

source("CPI-U Tips Comparison 2021-05-31.R")
...
# make final cash values df
  ...
  for(i in 7:9){
    ...
    if(i == 7){
      final.cash.values$TIPS.tax.rate <- abs(1 - temp)
    }
    if(i == 8){
      final.cash.values$Nom.tax.rate <- abs(1 - temp)
    }
    if(i == 9){
      final.cash.values$IB.tax.rate <- abs(1-temp)
    }
  }
...
best.return.NT <- vector()
  ...
  for(i in 1:nrow(final.cash.values)){
...
# plots
  ...
  lines(final.cash.values$Date, final.cash.values$Inflation, type ="l", col="blue", lwd=2, lty=2)
  lines(final.cash.values$Date, final.cash.values$IBNT, type ="l", col = "chartreuse3", lwd=2)
  abline(h=100, col="lightblue")
  ...
  plot(final.cash.values$Date, final.cash.values$TIPS.tax.rate, type="l", xaxt="n", xaxs="i", ylim=c(0.3,0.45),        # yields (amounts)
       ylab= "Effective Tax Rate", xlab="Date", lwd=2)
  title("Figure 3: Percent of Tax-Exempt Payoff that is Taxed")
  axis.Date(1, at=seq(min(TIPSdf$Date), max(TIPSdf$Date), by="year"))
  lines(final.cash.values$Date, final.cash.values$Nom.tax.rate, type ="l", col="red", lwd=2)
  lines(final.cash.values$Date, final.cash.values$IB.tax.rate, type ="l", col="blue", lwd=2)
...
## effective tax rate graphs:
  ...
  ## normalized graph. This should make more sense to people. 
