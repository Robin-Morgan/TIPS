...
TIPSdf$After_tax_yield <- c(after_tax_yield, NA)
TIPSdf$Mo_Comp_Infl_Rate <- c(comp_inflation_rate, NA)

#yields need fixing

after_tax_yield <- log(AIB/100)/(min(mat*12, nrow(TIPSdf)-i))
...
## Setting up after-tax yield to mat

TIPSdf$Cash_to_mat <- c(payoffstest, NA)                      # Final payoff
TIPSdf$Infl_to_end <- c(inflation_to_end, 100)                # Final inflated amount
TIPSdf$spread <- TIPSdf$Cash_to_mat - TIPSdf$Infl_to_end      # Spread
TIPSdf$After_tax_yield <- c(after_tax_yield, NA)              # after-tax yield
TIPSdf$Mo_Comp_Infl_Rate <- c(comp_inflation_rate, NA)        # "yield" of inflation


## after-tax TIPS yields vs inflation (monthly rate, continuously compounding interest)
  plot(TIPSdf$Date, TIPSdf$After_tax_yield, type="o", xaxt="n", xaxs="i", ylim=c(-0.002, 0.006),           # yields (amounts)
     ylab= "Yields (monthly)", xlab="Date")
  title("After-tax TIPS yields vs Inflation")
  lines(TIPSdf$Date, TIPSdf$Mo_Comp_Infl_Rate, type ="o", col="red")
  axis.Date(1, at=seq(min(TIPSdf$Date), max(TIPSdf$Date), by="year"))
  abline(h=0, col="lightblue")
  legend("topleft", legend=c("TIPS yields","Inflation"), col=c("black","red"), lty=1)

  
## like above but just plotting the spread
  plot(TIPSdf$Date, TIPSdf$After_tax_yield - TIPSdf$Mo_Comp_Infl_Rate, type="o", xaxt="n", xaxs="i", ylim=c(-0.004, 0.004),           # yields (amounts)
       ylab= "Yields (monthly)", xlab="Date")
  title("After-tax TIPS yields-Inflation Spread")
  axis.Date(1, at=seq(min(TIPSdf$Date), max(TIPSdf$Date), by="year"))
  abline(h=0, col="red")

pct_lose <- sum(TIPSdf$spread < 0, na.rm = TRUE)/nrow(TIPSdf) # % of losers
