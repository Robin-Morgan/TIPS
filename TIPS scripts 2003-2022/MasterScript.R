...
source("CPI-U Tips Comparison 2022-08-14.R")           # good (to delete)
  source("TIPS No Tax.R")                              # good (to delete)
...
par(family = "A")
  ...
  #plotting TIPS v Inflation Figure 1
  ...
  lines(final.cash.values$Date, final.cash.values$Inflation, type ="l", col=(alpha(rgb(0,0,0), 0.3)), lwd=2)
  title("Figure 1: Investing $100: TIPS vs Inflation")
  abline(h=100, col=(alpha(rgb(0,0,0), 0.2)))
  legend("topright", legend=c("TIPS", "Inflation"), col=c("black",col=(alpha(rgb(0,0,0), 0.3))), lty=c(1,1), 
         lwd=c(2,2))
  ...
  plot(final.cash.values$Date, final.cash.values$TIPSPayoff, type="l", xaxt="n", xaxs="i", ylim=c(95,140),        # yields (amounts)
       ylab= "Final Payouts", xlab="Date", lwd=2)
  title("Figure 3: Growth of $100 in TIPS, Nominal, I Bond, Inflation (2003-2022)")
  axis.Date(1, at=seq(min(TIPSdf$Date), max(TIPSdf$Date), by="year"))
  lines(final.cash.values$Date, final.cash.values$NomPayoff, type ="l", col=(alpha(rgb(0,0,0), 0.4)), lwd=2)      # replace with red
  lines(final.cash.values$Date, final.cash.values$Inflation, type ="l", col="chartreuse3", lwd=2, lty=2)
  lines(final.cash.values$Date, final.cash.values$IBPayoff, type ="l", col=(alpha(rgb(0,0,0), 0.15)), lwd=2)      # replace with blue
  abline(h=100, col=(alpha(rgb(0,0,0), 0.2)))
  legend("topright", legend=c("TIPS","Nominal","I Bonds", "Inflation"), col=c("black",col=(alpha(rgb(0,0,0), 0.4)),col=(alpha(rgb(0,0,0), 0.15)), "chartreuse3"), lty=c(1,1,1,2), 
         lwd=c(2,2,2,2))
  
  # plotting no tax
  plot(final.cash.values$Date, final.cash.values$TIPSNT, type="l", xaxt="n", xaxs="i", ylim=c(95,170),        # yields (amounts)
       ylab= "Final Payouts", xlab="Date", lwd=2)
  title("Payout of $100 Tax Exempt Investment: TIPS, Nominal, IBond, Inflation")
  axis.Date(1, at=seq(min(TIPSdf$Date), max(TIPSdf$Date), by="year"))
...
## effective tax rate graphs:
  ...
  plot(final.cash.values$Date, c(etr_TIPS_norm,NA), type="l", xaxt="n", xaxs="i", ylim=c(0.3,0.35),        # yields (amounts)
       ylab= "Effective Tax Rate", xlab="Date", lwd=2)
  title("Figure 4: Effective Tax Rate (Normalized)")
  axis.Date(1, at=seq(min(TIPSdf$Date), max(TIPSdf$Date), by="year"))
  lines(final.cash.values$Date, c(etr_Nom_norm,NA), type ="l", col = "black", lwd=2, lty = 2)
  lines(final.cash.values$Date, c(etr_IB_norm,NA), type ="l", col=(alpha(rgb(0,0,0), 0.3)), lwd=2, lty=1)
  legend("bottomright", legend=c("TIPS","Nominal","I Bonds"), col=c("black","black",col=(alpha(rgb(0,0,0), 0.3))), lty=c(1,2,1), 
         lwd=c(2,2,2))
