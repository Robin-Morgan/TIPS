...
Nomdf <- subset(tmp3, tmp3$DATE>=as.Date("2003-01-01") & tmp3$DATE <= as.Date("2022-01-01"))
...
for(i in 1:(nrow(Nomdf)-1)){
  ...
  if((i+ mat*12) > nrow(Nomdf)){
    ...
    # what was the compounded interest (rate) between the last interest payout and Jan 1 2022?
    rate_payment_2022 <- (1 + monthly_rate)^(nrow(Nomdf) - i - t*6)   
    
    # No Tax
    monthly_rate_NT <- (1 + yield)^(1/12) - 1
    rate_payment_2022_NT <- (1 + monthly_rate_NT)^(nrow(Nomdf) - i - t*6)
    
    if((i + (round_any(nrow(Nomdf) - i, 6, floor))) != 217){
    ATPayoff[i] <- AIB * (1+ATyield/2)^(t) + (AIB * (1+ATyield/2)^(t)) * (rate_payment_2022-1)
    NomPayoff_NT[i] <- AIB * (1+yield/2)^(t) + (AIB * (1+yield/2)^(t)) * (rate_payment_2022_NT-1)
    }else{
      ATPayoff[i] <- AIB * (1+ATyield/2)^(t)
      NomPayoff_NT[i] <- AIB * (1+yield/2)^(t) 
    }
